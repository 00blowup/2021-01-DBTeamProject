...
public class Rate_Num {
	...
	JTable table;
	JScrollPane js;
	String str[] = { "기수", "학번", "이름", "제목", "감독", "평점", "평론" }; // 컬럼명

	Rate_Num(Connection conn, Statement stmt) {
		this.conn = conn;
		this.stmt = stmt;
		...
		menu = new JLabel("기수별", JLabel.CENTER);
		menu.setFont(f1);
		menu.setBorder(new EmptyBorder(40, 0, 0, 0));
		mainPanel.add(menu);

		l1 = new JLabel("<기수별 평가>", JLabel.CENTER);
		l1.setFont(f1);
	...
	public void getTable() {
		...
		try {
			...
			ResultSet rs = stmt.executeQuery("SELECT 기수, 학번, 이름, 제목, 감독, 평점, 평론 FROM DB2021_평가 ORDER BY 기수 DESC");
			...
			while (rs.next()) {
				...
				Object data[] = { inception, id, name, title, director, score, review };
				model.addRow(data);
